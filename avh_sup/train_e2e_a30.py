"""
E2E Training — A30: Fine-tune from A9_e2e + Direct Mean Alignment on z_sync_pool
===================================================================================
...
class E2EModelA30(L.LightningModule):
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── A30 core: WHITENED MEAN ALIGNMENT (Mahalanobis) ──────────────────
        # Problem with A29: MSE(mu0, mu1) gradient shrinks with gap. Within-class
        # std=0.0019 but mean gap stuck at 0.018 → effect size 9.5σ. Convergence
        # slows to 1.2x/epoch because gradient ∝ gap.
        #
        # Fix: normalize the mean difference by the within-batch std (detached).
        # Loss = mean( ((mu0 - mu1) / std_within.detach()).^2 )
        #      = Cohen's d^2  (dimensionless, scale-invariant)
        # Gradient is now proportional to effect size not absolute gap →
        # convergence rate is independent of representation scale.
        if n0 >= 1 and n1 >= 1:
            mu0 = z_sync_pool[mask0].mean(0)   # [512]
            mu1 = z_sync_pool[mask1].mean(0)   # [512]
            with torch.no_grad():
                std_within = z_sync_pool.std(0).clamp(min=1e-6)  # [512], detached
            loss_mean_align = ((mu0 - mu1) / std_within).pow(2).mean()
        else:
...
def main():
    ...
    model = E2EModelA30(config=config)
    ...
    if finetune_ckpt and os.path.isfile(finetune_ckpt):
        print(f"[A30] Loading weights from {finetune_ckpt} (strict=False)", flush=True)
        ...
        print(f"[A30] Missing keys (new modules, OK): {missing[:10]}", flush=True)
        print(f"[A30] Unexpected keys (removed modules, OK): {unexpected[:5]}", flush=True)
    else:
        print("[A30] WARNING: no finetune_ckpt found — starting from scratch.", flush=True)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A30_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A30_e2e/ckpts"),
    ...
    print(">>> A30_e2e training complete.", flush=True)
