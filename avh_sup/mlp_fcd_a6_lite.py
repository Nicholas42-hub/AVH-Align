"""
AVH-Align A6-lite — FCD + Light Domain Adversarial with Warmup
===============================================================

Extends A6 with two modifications:
  1. Reduced domain loss weights: lambda_dadv = lambda_ddis = 0.2 (vs 1.0 in A6)
  2. Warmup epochs: domain losses are zeroed for the first `warmup_epochs` epochs,
     letting the task pathway stabilise before domain pressure is applied.

Rationale: A6's drop under leakage-control suggests domain gradient is too
early and too strong, interfering with the unique routing (u_v, u_a) that
provides the real discriminative value.
...
class AVH_FCD_A6_Lite(L.LightningModule):
    """
    A6-lite: same architecture as A6 but with:
      - Reduced lambda_dadv / lambda_ddis (default 0.2 each)
      - Domain losses suppressed for the first `warmup_epochs` epochs
    """
    ...
    def __init__(self, config: dict):
        ...
        # ── A6 domain heads (same dims as A6) ────────────────────────────────
        ...
        self.lambda_dadv   = float(hp.get("lambda_dadv",   0.2))  # reduced
        self.lambda_ddis   = float(hp.get("lambda_ddis",   0.2))  # reduced
        self.grl_alpha     = float(hp.get("grl_alpha",     1.0))
        self.warmup_epochs = int  (hp.get("warmup_epochs", 5))     # new
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Domain losses — suppressed during warmup ──────────────────────────
        in_warmup = self.current_epoch < self.warmup_epochs
        if in_warmup:
            loss_dadv = torch.tensor(0.0, device=video_feats.device)
            loss_ddis = torch.tensor(0.0, device=video_feats.device)
        else:
        ...
        self.log("train_in_warmup",      float(in_warmup), **log_kw)
        return loss
    ...
    def on_validation_epoch_end(self):
        ...
        warmup_str = f" [WARMUP {self.current_epoch+1}/{self.warmup_epochs}]" \
            if self.current_epoch < self.warmup_epochs else ""
        print(
            f"\n[Epoch {self.current_epoch}]{warmup_str}"
