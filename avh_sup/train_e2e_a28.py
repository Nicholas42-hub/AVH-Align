"""
E2E Training — A28: Fine-tune from A9_e2e + Direct Mean Alignment on z_sync_pool
===================================================================================
...
**Expected dynamics**:
  ...
  python train_e2e_a28.py --config_path configs/A26_e2e.yaml
"""
...
class E2EModelA28(L.LightningModule):
    """
...
def main():
    ...
    model = E2EModelA28(config=config)
    ...
    if finetune_ckpt and os.path.isfile(finetune_ckpt):
        print(f"[A28] Loading weights from {finetune_ckpt} (strict=False)", flush=True)
        ...
        print(f"[A28] Missing keys (new modules, OK): {missing[:10]}", flush=True)
        print(f"[A28] Unexpected keys (removed modules, OK): {unexpected[:5]}", flush=True)
    else:
        print("[A28] WARNING: no finetune_ckpt found — starting from scratch.", flush=True)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A28_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A28_e2e/ckpts"),
    ...
    print(">>> A28_e2e training complete.", flush=True)
