"""
E2E Training — A21: BatchNorm1d on z_sync_pool + Strong Dual DANN
================================================================================

A21 = A20 (dual DANN, lambda_ddis=0) + two key improvements:

1. BatchNorm1d(512) on z_sync_pool BEFORE the domain DANN loss and probe.
   Rationale: domain AUC=1.0 most likely reflects first-order statistics
   (mean/variance shift across AV1M vs FakeAVCeleb). BatchNorm explicitly
   normalises each feature to mean=0, var=1 across the batch, destroying
   the first-order component that a linear probe exploits trivially.
   BN gradient flows back through z_sync to the encoder, teaching it to
   produce domain-invariant pooled features.

2. lambda_enc_dann=15.0 and lambda_zsync_dann=15.0 (up from 1.0).
   With 325M encoder vs a ~260K-param classifier, lambda=1.0 is negligible.
   At lambda=15, the domain-reversal gradient is 15× the task gradient,
   providing enough pressure to actually move the encoder.

  python train_e2e_a21.py --config_path configs/A21_e2e.yaml
"""
...
class E2EModelA21(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A21: BatchNorm1d on z_sync_pool + strong dual DANN (lambda=15).
    BatchNorm destroys first-order domain statistics; DANN kills orientation.
    lambda_ddis=0.0 keeps gradients uncontested.
    """
    ...
    def __init__(self, config: dict):
        ...
        # A21 new: BatchNorm1d on z_sync_pool to destroy first-order domain stats
        self.bn_zsync = torch.nn.BatchNorm1d(zsync_dim, track_running_stats=True)

        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A21
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Encoder-level DANN (A17 component): GRL on enc_feats ───────────
        ...
        # ── Z_sync-level DANN (A21): GRL on BN-normalised z_sync_pool ─────────
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [BN+dual DANN lambda=15, ddis=0]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA21(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A21_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A21_e2e/ckpts"),
    ...
    print(">>> A21_e2e training complete.", flush=True)
