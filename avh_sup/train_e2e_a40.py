"""
E2E Training — A40: A39 + Z_c Mean Alignment
=============================================

**Context: A39 diagnosis**

  A39 (main classification on Z_c + label-aware domain pull on Z_s, no IPW/GRL)
  ran for 16 epochs and showed:
    domain_auc_zc_raw      = 1.000  ← BAD: Z_c still fully domain-discriminative
    domain_auc_zc_centered = 0.014  ← GOOD: Z_c covariance is already domain-invariant
    domain_auc_zs_raw      = 1.000  ← expected: Z_s absorbs domain (pull working)

  Because zc_centered ≈ 0.5 but zc_raw = 1.0, the ONLY domain information
  remaining in Z_c is a per-dimension MEAN OFFSET between domains. A logistic
  regression can perfectly separate domains using only this mean shift.

**A40 fix: one targeted addition**

  L_mean_zc = MSE(mean(Z_c[domain=0]), mean(Z_c[domain=1]))  per batch

  This directly closes the mean gap between domains in Z_c. Gradients flow
  through visual_proj_causal and audio_proj_causal (inside AVH_Causal_Ablation)
  and all unfrozen AV-HuBERT layers.

  The label-aware domain pull on Z_s (ladv_ds from A39) is retained as-is,
  so domain information has a "sink" in Z_s while Z_c means are equalized.

  IPW and GRL remain disabled (lambda=0.0) — same as A39.

**Full loss (A40):**
  L = L_task                         (Z_c classification, AV1M only)
    + lambda_ladv_ds * L_ladv_ds     (label-aware domain pull on Z_s)
    + lambda_mean_zc * L_mean_zc     (NEW: Z_c mean alignment)
    + lambda_orth    * L_orth        (optional: Z_c ⊥ Z_s on AV1M only)

**Success criteria:**
  domain_auc_zc_raw       < 0.65   (true domain invariance after mean alignment)
  domain_auc_zc_centered  < 0.10   (covariance already clean, should stay so)
  domain_auc_zs_raw       > 0.80   (Z_s domain sink still working)
  val_auc_causal          > 0.95   (task not hurt by mean alignment)

  python train_e2e_a40.py --config_path configs/A40_e2e.yaml
"""
...
class BalancedDomainSampler(Sampler):
    ...
    batch_size/2 FAVC clips. This ensures every batch has both domains
    for the Z_c mean alignment and Z_s domain pull losses.
    """
...
class E2EModelA40(L.LightningModule):
    """
    A40: Z_c classification-only + label-aware domain pull on Z_s + Z_c mean alignment.
    ...
    Architecture:
      ...
      Z_s [B, T, spurious_dim] → cond_domain_head_s → label-aware domain pull

    Losses (all from A39 + one addition):
      L_task     : CE on Z_c (AV1M only)
      L_ladv_ds  : label-aware BCE on Z_s  (routes domain signal INTO Z_s)
      L_mean_zc  : MSE between Z_c domain means  (NEW — closes mean gap in Z_c)
    ...
    def __init__(self, config: dict):
        ...
        # ── Label-aware domain pull on Z_s (retained from A39) ────────────────
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Keep this on the labelled/source subset so the orth variant differs
        # from A40 only by one extra regularizer, matching older AVH-Align runs.
        ...
        # ── 2. Label-aware domain pull on Z_s (from A39) ──────────────────────
        # y_hat_prob: model's predicted fake probability (stop-grad).
        # Conditioning on class prevents the domain head from "cheating"
        # by exploiting class-correlated features inside Z_s.
        with torch.no_grad():
        ...
        # ── 3. Z_c mean alignment (NEW in A40) ───────────────────────────────
        # Equalise the per-dimension mean of Z_c between both domains.
        # Because zc_centered ≈ 0.014 in A39, this is the ONLY remaining
        # domain signal in Z_c — pushing means together should drop raw AUC.
        mask_d0 = (domain_labels == 0)
        mask_d1 = (domain_labels == 1)
        if mask_d0.any() and mask_d1.any():
            mean_d0 = z_c_pool[mask_d0].mean(0)   # [causal_dim]
            mean_d1 = z_c_pool[mask_d1].mean(0)   # [causal_dim]
...
def main():
    ...
    model = E2EModelA40(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A40_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A40_e2e/ckpts"),
