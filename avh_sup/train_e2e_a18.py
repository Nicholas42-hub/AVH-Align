"""
E2E Training — A18 + Dual DANN at Encoder AND Z_sync levels
================================================================================

A18 extends A17 by adding a SECOND domain adversarial classifier directly on
Z_sync (the 512-dim AV-sync representation), in addition to the encoder-level
DANN from A17.

Motivation:
  A17 applies GRL only at enc_feats [B,2048].  Even if the encoder becomes
  domain-agnostic, the SAD head could potentially re-encode domain information
  into z_sync from the (still somewhat domain-predictive) encoder outputs.
  A18 closes this gap by ALSO applying GRL at z_sync_pool [B,512], the exact
  representation whose domain AUC we want to reduce.

Dual DANN architecture:
  • Level 1 (A17 component): GRL on enc_feats [2048]
      domain_clf_enc: [2048] → 256 → LayerNorm → ReLU → 64 → ReLU → 1
      Gradient reversed through encoder weights directly.

  • Level 2 (A18 new component): GRL on z_sync_pool [512]
      domain_clf_zsync: [512] → 256 → LayerNorm → ReLU → 64 → ReLU → 1
      Gradient reversed through SAD head → encoder.

  Combined, the encoder receives adversarial gradient from BOTH paths:
  - Direct path (enc_feats GRL → encoder)
  - Indirect path (z_sync GRL → SAD head → encoder)

  The SAD head ALSO receives adversarial gradient (z_sync GRL path),
  forcing it not to encode domain info into z_sync.

  Monitor:
    train_domain_acc_enc   → 0.5 means encoder is domain-invariant
    train_domain_acc_zsync → 0.5 means z_sync is domain-invariant

  python train_e2e_a18.py --config_path configs/A18_e2e.yaml
"""
...
class E2EModelA18(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A18: Dual DANN — GRL at both enc_feats [2048] and z_sync_pool [512].
    Belt+suspenders: attacks domain at encoder level + z_sync level.
    """
    ...
    def __init__(self, config: dict):
        ...
        # A18: domain classifier at enc_feats [2048] level with GRL (from A17)
        ...
        # A18 new: domain classifier at z_sync_pool [512] level with GRL
        zsync_dim = int(hp.get("sync_dim", 512))  # 512
        self.domain_clf_zsync = _make_adv_head(zsync_dim)
        self.lambda_zsync_dann = float(hp.get("lambda_zsync_dann", 1.0))
        self.grl_alpha_zsync   = float(hp.get("grl_alpha_zsync",   1.0))

        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A18
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Z_sync-level DANN (A18 new): GRL on z_sync_pool ──────────────────
        z_sync_pool_grl    = grad_reverse(z_sync_pool, alpha=self.grl_alpha_zsync)
        ...
        # Optional CORAL secondary term (lambda_enc_coral=0 in A18 config)
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [enc+zsync dual DANN]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA18(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A18_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A18_e2e/ckpts"),
    ...
    print(">>> A18_e2e training complete.", flush=True)
