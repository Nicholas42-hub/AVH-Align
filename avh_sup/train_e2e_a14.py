"""
E2E Training — A14 + Encoder-Level CORAL + Higher Encoder LR, Fully End-to-End
==============================================================================

Difference from A11:
  Same CORAL on encoder output (lambda_enc_coral=25).
  lr_encoder raised from 1e-6 → 5e-6 (5× higher).

  Hypothesis: with lr_encoder=1e-6 the encoder weights barely move per step.
  If CORAL gradient is small (small distribution gap in 16-bit, batch_size=8),
  the encoder may not actually become domain-invariant.
  A higher encoder LR amplifies the CORAL signal relative to task loss.

  Risk: encoder might forget pre-trained features → watched via task AUC.

  python train_e2e_a14.py --config_path configs/A14_e2e.yaml
"""
...
class E2EModelA14(L.LightningModule):
    """
    ...
    def __init__(self, config: dict):
        ...
        # A14: lr_encoder default raised to 5e-6 (config key: lr_encoder)
    ...
    def configure_optimizers(self):
        ...
        lr_encoder   = float(hp.get("lr_encoder",   5e-6))  # A14: 5e-6 vs 1e-6 in A11
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [lr_enc=5e-6]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA14(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A14_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A14_e2e/ckpts"),
    ...
    print(">>> A14_e2e training complete.", flush=True)
