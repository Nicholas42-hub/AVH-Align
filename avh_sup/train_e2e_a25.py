"""
E2E Training — A25: Per-Domain Mean Centering of z_sync_pool + DANN lambda=10
================================================================================

**Motivation**:
  The simplest reason a linear probe achieves AUC=1.0 is that the two domains
  have different MEAN vectors in z_sync space.  A probe trained on mean-shifted
  features can reach 100% AUC by a mere threshold on the first principal
  component.  BN (A21) removes this mean shift DURING TRAINING by normalizing
  the batch, but at inference time the running mean may not generalize perfectly.

  A25 takes a MORE DIRECT approach: explicitly subtract per-domain mean from
  z_sync_pool within each training batch.  This is a clean, interpretable
  intervention that GUARANTEES mean_0 = mean_1 = 0 within every training batch.
  The encoder must therefore learn to produce features where the RESIDUAL
  (within-domain variance) is not domain-discriminative, which is a strictly
  harder constraint than just having similar means.

**Mechanism**:
  Within each training step:
    z_c[i] = z_sync_pool[i] - mean_d  where d = domain_label[i] ∈ {0,1}
  BalancedDomainSampler guarantees n0 = n1 = 4, so batch_mean_0 and batch_mean_1
  are computed from the same number of samples (avoids estimation bias).

  Gradient flows THROUGH the centering operation:
    d(loss)/d(z_sync_pool[i]) = d(loss)/d(z_c[i]) * (1 - 1/n_d)
    (standard gradient of mean-subtraction; encoder learns to reduce residual domain signal)

**Combined with DANN** (lambda=15 on both enc and z_sync levels, updated from 10
  based on A18 showing lambda=1 dual DANN regressed after epoch 0):
  Centering removes the mean component; DANN adversarially removes orientation
  differences in the RESIDUAL.  Together they address both first- and higher-order
  domain shifts.  Both push the encoder toward fully domain-invariant features.

  lambda_ddis=0.0, lambda_orth=0.1 (same as A20-A24).

  python train_e2e_a25.py --config_path configs/A25_e2e.yaml
"""
...
class E2EModelA25(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A25: Per-domain batch mean centering of z_sync_pool + dual DANN (lambda=10).

    Centering step (in training_step):
      z_c[mask_d] = z_sync_pool[mask_d] - z_sync_pool[mask_d].mean(0)
    This GUARANTEES mean_0 == mean_1 == 0 within every training batch, making
    it impossible for the task head or domain probe to exploit mean differences.
    Gradient flows through this operation back to the encoder.

    Dual DANN (enc + zsync, lambda=10) adversarially removes residual orientation
    differences after centering.  Together they target both first- and higher-order
    domain shifts simultaneously.

    Note: at inference / probe time, centering is applied using the BATCH mean
    of whatever samples are in the probe loader batch (both domains mixed, BDS
    ensures 50/50 split in probe loader too). This may differ from training-time
    centering slightly, but the probe reports the AUC on centered features.
    """
    ...
    def __init__(self, config: dict):
        ...
        # A25: use stronger DANN (lambda=10) on both enc and zsync
        # Centering (applied in training_step) removes first-order stats;
        # DANN removes residual orientation differences.
        zsync_dim = int(hp.get("sync_dim", 512))  # 512
        self.domain_clf_zsync = _make_adv_head(zsync_dim)
        self.lambda_zsync_dann = float(hp.get("lambda_zsync_dann", 10.0))
        self.grl_alpha_zsync   = float(hp.get("grl_alpha_zsync",   1.0))

        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A25
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Per-domain mean centering of z_sync_pool (A25 core contribution) ────
        # Subtract per-domain batch mean: this GUARANTEES mean_d=0 for both
        # domains in every batch, so a linear probe cannot exploit mean shift.
        # Gradient of centering w.r.t. z_sync_pool[i] = I - (1/n_d) * 11^T
        # (standard mean-subtraction Jacobian) → flows to encoder.
        ...
        # ── Encoder-level DANN (upstream pressure, lambda=10) ────────────────
        ...
        # ── Z_sync-level DANN on CENTERED z_sync_pool ───────────────────────
        # DANN operates on the centered representation, so it adversarially
        # removes the RESIDUAL domain signal beyond the mean shift.
        ...
        with torch.no_grad():
            domain_acc_zsync = ((domain_logit_zsync.detach() > 0).float() == domain_labels).float().mean()
    ...
    def _run_domain_probe(self):
        ...
        # A25: apply the same per-domain centering as in training_step
        # This shows whether the CENTERED representation is domain-discriminative.
        for d in [0, 1]:
            idx_d = (dom_all == d)
            if idx_d.sum() > 1:
                zs_all[idx_d] = zs_all[idx_d] - zs_all[idx_d].mean(0)
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [per-domain centering + dual DANN lambda=10, ddis=0]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA25(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A25_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A25_e2e/ckpts"),
    ...
    print(">>> A25_e2e training complete.", flush=True)
