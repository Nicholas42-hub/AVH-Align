"""
E2E Training — A22: BatchNorm1d on BOTH enc_feats AND z_sync_pool + Strong Dual DANN
================================================================================

A22 = A21 + extend BatchNorm normalisation to the upstream encoder level.

**Why A21 may not be enough**:
  A21 applies BN on z_sync_pool [B,512] (the DANN target).  But z_sync is
  produced by a learned projection from video_feats+audio_feats.  If enc_feats
  [B,2048] still carries a strong mean/variance domain shift, the SAD head
  projection may re-bake that shift into z_sync even after BN on z_sync_pool.
  The BN gradient in A21 only sees z_sync, not enc_feats directly.

**A22 adds**:
  1. BatchNorm1d(2048) on enc_feats BEFORE the encoder-level DANN classifier
     and BEFORE enc_feats is passed to the SAD head.  This removes the
     first-order domain signal at the encoder output level, so the SAD
     projection receives domain-centred inputs.
     → Gradient of BN flows back through enc_feats to the AV-HuBERT encoder.

  2. BatchNorm1d(512) on z_sync_pool (same as A21) for the z_sync DANN path.

  3. lambda_enc_dann=15.0, lambda_zsync_dann=15.0 (same as A21) for sufficient
     gradient magnitude against the 325M-param encoder.

  4. lambda_ddis=0.0 (no conflicting domain gradient from z_app path).

**Expected outcome**:
  If domain AUC=1.0 is driven by a mean shift in enc_feats that propagates
  through the SAD projection, BN at enc level will break that path directly.
  Combined with BN on z_sync, both levels are normalised simultaneously.
...
class E2EModelA22(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A22: BatchNorm1d on BOTH enc_feats [B,2048] AND z_sync_pool [B,512],
         combined with strong dual DANN (lambda=15) on both levels.

    Two-level BN strategy:
      - bn_enc  (BN1d 2048): normalises mean/var of encoder outputs before SAD
                              head projection AND before enc-level DANN.
      - bn_zsync (BN1d 512) : normalises mean/var of z_sync_pool before
                              zsync-level DANN and domain probe.

    lambda_ddis=0.0 removes the conflicting gradient that would push domain
    info BACK into the encoder (see A20 for motivation).
    """
    ...
    def __init__(self, config: dict):
        ...
        self.lambda_enc_dann = float(hp.get("lambda_enc_dann", 15.0))
        self.grl_alpha_enc   = float(hp.get("grl_alpha_enc",   1.0))

        # A20 new: domain classifier at z_sync_pool [512] level with GRL
        zsync_dim = int(hp.get("sync_dim", 512))  # 512
        self.domain_clf_zsync = _make_adv_head(zsync_dim)
        self.lambda_zsync_dann = float(hp.get("lambda_zsync_dann", 15.0))
        self.grl_alpha_zsync   = float(hp.get("grl_alpha_zsync",   1.0))

        # A21 component: BN on z_sync_pool [B,512] — removes first-order stats at SAD level
        self.bn_zsync = torch.nn.BatchNorm1d(zsync_dim, track_running_stats=True)

        # A22 addition: BN on enc_feats [B,2048] — removes first-order stats at ENCODER level
        # This ensures the SAD head projection receives domain-centred inputs,
        # preventing enc-level domain mean/var from being re-injected into z_sync.
        enc_dim2 = 2 * int(hp.get("feat_dim", 1024))  # 2048
        self.bn_enc = torch.nn.BatchNorm1d(enc_dim2, track_running_stats=True)

        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A22
    ...
    def training_step(self, batch, batch_idx):
        ...
        z_sync_pool = z_sync.mean(1)                      # [B, 512] raw
        z_sync_pool_bn = self.bn_zsync(z_sync_pool)       # [B, 512] BN-normalised
        ...
        # ── Encoder-level DANN (A22: GRL on BN-normalised enc_feats) ─────────
        # BN on enc_feats: removes per-feature mean/var domain shift at encoder output.
        # The BN gradient propagates back through enc_feats to AV-HuBERT weights.
        enc_feats     = torch.cat([video_feats.mean(1), audio_feats.mean(1)], dim=-1)  # [B, 2048]
        enc_feats_bn  = self.bn_enc(enc_feats)          # [B, 2048], mean=0 var=1 per feature
        enc_feats_grl = grad_reverse(enc_feats_bn, alpha=self.grl_alpha_enc)
        ...
        # ── Z_sync-level DANN (A21/A22): GRL on BN-normalised z_sync_pool ────────
        # BN kills first-order stats; DANN kills orientation differences.
        z_sync_pool_grl    = grad_reverse(z_sync_pool_bn, alpha=self.grl_alpha_zsync)
        domain_logit_zsync = self.domain_clf_zsync(z_sync_pool_grl).squeeze(-1)  # [B]
        ...
        # Optional CORAL secondary term (lambda_enc_coral=0 in A20 config)
    ...
    def _run_domain_probe(self):
        ...
        for batch in self.domain_probe_loader:
            ...
            enc_feats_p = self.bn_enc(enc_feats)   # eval-mode BN (running stats)
            enc_list.append(enc_feats_p.cpu().numpy())
            z_sync_pool_p = self.bn_zsync(z_sync.mean(1))
            zs_list.append(z_sync_pool_p.cpu().numpy())
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [BN-enc+BN-zsync+dual DANN lambda=15, ddis=0]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA22(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A22_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A22_e2e/ckpts"),
    ...
    print(">>> A22_e2e training complete.", flush=True)
