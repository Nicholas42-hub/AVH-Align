"""
E2E Domain & Label Probe Evaluation — A10_e2e (SAD + CORAL + Balanced Batching)
=================================================================================
Evaluates an E2EModelA10 checkpoint on:
  1. Domain predictability (AV1M=0 vs FakeAVCeleb=1) from Z_sync and Z_app
     → target: Z_sync AUC ≈ 0.5  (CORAL expelled domain through encoder)
  2. Label AUC (real=0 vs fake=1) from Z_sync and Z_app on FAVC (OOD)
     → tests whether Z_sync generalises out-of-domain

Key differences from A9_e2e:
  - CORAL loss (mean + covariance matching) replaces multi-kernel MMD
  - BalancedDomainSampler guarantees 50/50 AV1M/FAVC per batch (100% fire rate)
  - Uses ALL FAVC clips (real+fake), 3× oversample → balanced domain batches

Comparison baselines (frozen encoders):
  Raw AV-HuBERT:          domain AUC ≈ 0.999
  A8 Z_sync (GRL):        domain AUC ≈ 0.986  Δraw = −0.014
  A9 Z_sync (frozen MMD): domain AUC ≈ 0.988  Δraw = −0.011
  A9_e2e Z_sync (MMD e2e):domain AUC ≈ 0.989  Δraw = −0.010
  A10_e2e Z_sync target:  domain AUC < 0.65   (CORAL + balanced batching)

Usage:
  python eval_e2e_domain_a10.py \\
      --ckpt      outputs_A10_e2e/ckpts/model-epoch=XX-val_auc_causal=X.ckpt \\
      --config    configs/A10_e2e.yaml \\
      --av1m_raw  /data/scratch/.../avd1m_preprocessed \\
      --favc_raw  /data/scratch/.../favc_preprocessed \\
      --av1m_csv  csv_metadata/av1m \\
      --output_dir outputs_A10_e2e/results \\
...
from train_e2e_a10 import E2EModelA10
...
@torch.no_grad()
def extract_zc_zs(model: E2EModelA10, dataloader: DataLoader,
                  device: torch.device, domain_label: int):
    """
    Run raw video/audio through the full e2e pipeline.
    Returns mean-pooled Z_sync (causal), Z_app (spurious) [N, D]
    and per-sample cls/domain labels.
    """
    model.eval()
    model.to(device)
    ...
    for batch in tqdm(dataloader, leave=False):
        ...
        # SAD decomposition → Z_sync [B,T,sync_dim], Z_app [B,T,app_dim*2]
        z_sync, z_app, _ = model.head._encode(video_feats, audio_feats)

        # Mean-pool over time → [B, D]
        zc = z_sync.mean(dim=1).cpu().numpy()
        zs = z_app.mean(dim=1).cpu().numpy()
...
def linear_probe_auc(features: np.ndarray, labels: np.ndarray,
    ...
    for tr, te in skf.split(features, labels):
        ...
        clf = LogisticRegression(max_iter=1000, C=1.0).fit(
            sc.transform(features[tr]), labels[tr]
        )
...
def main():
    ...
    parser.add_argument("--output_dir",  default="outputs_A10_e2e/results")
    ...
    model = E2EModelA10.load_from_checkpoint(args.ckpt, config=config)
    model.eval()
    model.to(device)
    ...
    print(f"  Z_sync domain AUC: {dom_zc_mean:.4f} ± {dom_zc_std:.4f}")
    print(f"  Z_app  domain AUC: {dom_zs_mean:.4f} ± {dom_zs_std:.4f}")
    print(f"  → Z_sync domain-neutral? {'YES' if dom_zc_mean < 0.65 else 'NO'} "
          f"(target < 0.65, got {dom_zc_mean:.4f})")
    ...
    print(f"  Z_sync label AUC (in-domain):  {lbl_zc_id_mean:.4f} ± {lbl_zc_id_std:.4f}")
    print(f"  Z_app  label AUC (in-domain):  {lbl_zs_id_mean:.4f} ± {lbl_zs_id_std:.4f}")
    ...
    if n_real > 0 and n_fake > 0 and len(np.unique(cls_favc_v)) > 1:
        ...
        print(f"  Z_sync label AUC (OOD FAVC):   {lbl_zc_ood_mean:.4f} ± {lbl_zc_ood_std:.4f}")
        print(f"  Z_app  label AUC (OOD FAVC):   {lbl_zs_ood_mean:.4f} ± {lbl_zs_ood_std:.4f}")
    else:
    ...
    results = {
        ...
        "domain_probe": {
            "Z_sync_auc_mean": dom_zc_mean,
            "Z_sync_auc_std":  dom_zc_std,
            "Z_app_auc_mean":  dom_zs_mean,
            "Z_app_auc_std":   dom_zs_std,
        },
        "label_probe_indomain": {
            "Z_sync_auc_mean": lbl_zc_id_mean,
            "Z_sync_auc_std":  lbl_zc_id_std,
            "Z_app_auc_mean":  lbl_zs_id_mean,
            "Z_app_auc_std":   lbl_zs_id_std,
        },
        "label_probe_ood_favc": {
            "Z_sync_auc_mean": lbl_zc_ood_mean,
            "Z_sync_auc_std":  lbl_zc_ood_std,
            "Z_app_auc_mean":  lbl_zs_ood_mean,
            "Z_app_auc_std":   lbl_zs_ood_std,
        },
    }

    out_path = os.path.join(
        args.output_dir,
        "eval_e2e_domain_holdout.json" if args.holdout_only else "eval_e2e_domain.json"
    )
    ...
    # ── Summary table ─────────────────────────────────────────────────────────
    print("\n" + "="*65)
    print("SUMMARY — A10_e2e")
    print("="*65)
    print(f"{'Metric':<40} {'Z_sync':>8}  {'Z_app':>8}")
    print(f"{'Domain AUC (AV1M vs FAVC)':<40} {dom_zc_mean:>8.4f}  {dom_zs_mean:>8.4f}")
    print(f"{'Label AUC in-domain (AV1M val)':<40} {lbl_zc_id_mean:>8.4f}  {lbl_zs_id_mean:>8.4f}")
    if lbl_zc_ood_mean is not None:
        print(f"{'Label AUC OOD (FAVC)':<40} {lbl_zc_ood_mean:>8.4f}  {lbl_zs_ood_mean:>8.4f}")
    print("="*65)
    print("\nFrozen baselines (for reference):")
    print("  Raw AV-HuBERT  domain AUC ≈ 0.999")
    print("  A9 frozen MMD  domain AUC ≈ 0.988  Δraw = −0.011")
    print("  A9_e2e MMD     domain AUC ≈ 0.989  Δraw = −0.010  (broken batching)")
    print("  A10_e2e target domain AUC < 0.65   (CORAL + balanced batching)")
