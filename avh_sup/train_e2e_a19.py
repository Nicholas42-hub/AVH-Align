"""
E2E Training — A19 + LayerNorm + Dual DANN (Encoder AND Z_sync)
================================================================================

A19 = A18 (dual DANN) + A16 (LayerNorm on encoder output).

The combination is synergistic:
  • LayerNorm (deterministic, fast): removes absolute feature-scale domain
    signal immediately from epoch 0.  Eliminates recording quality, brightness,
    and energy-level confounds before they enter the adversarial game.

  • Dual DANN (adaptive, learned): attacks residual domain signal that persists
    after normalization.  The arms-race is now fighting smaller domain signal,
    so it should converge faster and achieve lower final domain AUC.

Gradient paths:
  • LayerNorm: no learnable parameters; gradient flows straight through.
  • enc_feats GRL: reversed gradient from domain_clf_enc to encoder.
  • z_sync_pool GRL: reversed gradient from domain_clf_zsync to SAD head + encoder.

  python train_e2e_a19.py --config_path configs/A19_e2e.yaml
"""
...
class E2EModelA19(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A19: LayerNorm on encoder output + Dual DANN (enc_feats + z_sync_pool).
    """
    ...
    def __init__(self, config: dict):
        ...
        # A19 (from A17/A18): domain classifier at enc_feats [2048] level with GRL
        ...
        # A19 (from A18): domain classifier at z_sync_pool [512] level with GRL
        ...
        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A19
    ...
    def training_step(self, batch, batch_idx):
        ...
        # A19: LayerNorm on encoder output (removes absolute-scale domain signal)
        video_feats = F.layer_norm(video_feats, [video_feats.shape[-1]])
        ...
        # ── Z_sync-level DANN (A19): GRL on z_sync_pool ──────────────────
        ...
        # Optional CORAL secondary term (lambda_enc_coral=0 in A19 config)
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [LN+enc+zsync dual DANN]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA19(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A19_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A19_e2e/ckpts"),
    ...
    print(">>> A19_e2e training complete.", flush=True)
