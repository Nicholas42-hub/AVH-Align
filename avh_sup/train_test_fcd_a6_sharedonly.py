"""
A6-sharedonly-trainreal — entry-point.

FCD + Domain Adversarial on shared subspace only (s_v, s_a), protecting u_v/u_a.
Domain source: FAVC train-real clips.
"""
...
def train(config: dict):
    ...
    model  = AVH_FCD_A6_SharedOnly(config=config)
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 68)
    print("  A6-sharedonly-trainreal — domain pressure on s_v/s_a only")
    print(f"  seed={config.get('seed', 43)}")
    print("  domain_head_c target: shared (s_v, s_a) [512-dim]")
    print("  domain_head_s target: spurious (r_v, r_a) [512-dim]")
    print("  u_v, u_a: PROTECTED from domain gradient")
    if args.output_dir is not None:
