"""
E2E Training — A16 + LayerNorm on Encoder Output + Encoder-Level CORAL
=======================================================================

Key addition vs A11:

  A16 applies LayerNorm to the AV-HuBERT encoder output (video_feats and
  audio_feats) BEFORE the SAD head:

    video_feats = F.layer_norm(video_feats, [video_feats.shape[-1]])
    audio_feats = F.layer_norm(audio_feats, [audio_feats.shape[-1]])

  LayerNorm normalises each token’s feature vector to (mean=0, std=1).
  This directly removes the dominant source of domain signal:
    - Recording brightness/contrast  (video feature scale)
    - Audio energy level/SNR         (audio feature scale)
  These are purely domain-confounds; the SAD head should not see them.

  Crucially, LayerNorm is:
    1. Deterministic — no noise, no per-batch statistics
    2. Applied identically at inference — no train/test mismatch
    3. Zero additional parameters
    4. Differentiable — gradients flow through to the encoder

  Combined with A11’s encoder-level CORAL (belt + suspenders):
    - LayerNorm removes absolute scale domain signal (deterministic)
    - CORAL removes residual 1st+2nd order statistics (soft penalty)"""  # noqa
...
class E2EModelA16(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A16: Apply LayerNorm to encoder output (video/audio feats) before SAD head.
    Also retains encoder-level CORAL from A11 (belt + suspenders).
    """
    ...
    def training_step(self, batch, batch_idx):
        ...
        # A16: per-token LayerNorm — removes absolute scale (main domain signal)
        video_feats = F.layer_norm(video_feats, [video_feats.shape[-1]])
        audio_feats = F.layer_norm(audio_feats, [audio_feats.shape[-1]])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        video_feats, audio_feats = self.encoder(video_raw, audio_raw)
        # A16: same LN as training — no train/test mismatch
        video_feats = F.layer_norm(video_feats, [video_feats.shape[-1]])
        audio_feats = F.layer_norm(audio_feats, [audio_feats.shape[-1]])
    ...
    def _run_domain_probe(self):
        ...
        for batch in self.domain_probe_loader:
            ...
            video_feats, audio_feats = self.encoder(video_raw, audio_raw)
            # A16: apply LN in probe too for consistent feature distribution
            video_feats = F.layer_norm(video_feats, [video_feats.shape[-1]])
            audio_feats = F.layer_norm(audio_feats, [audio_feats.shape[-1]])
...
def main():
    ...
    model = E2EModelA16(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A16_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A16_e2e/ckpts"),
    ...
    print(">>> A16_e2e training complete.", flush=True)
