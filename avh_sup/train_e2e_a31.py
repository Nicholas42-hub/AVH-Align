"""
E2E Training — A31: Fine-tune from A9_e2e + Direct Mean Alignment on z_sync_pool
===================================================================================
...
**Expected dynamics**:
  ...
  python train_e2e_a30.py --config_path configs/A26_e2e.yaml
"""
...
class E2EModelA31(L.LightningModule):
    """
    ...
    def __init__(self, config: dict):
        ...
        self.lambda_mean_align = float(hp.get("lambda_mean_align",  0.05))

        self.lambda_ddis     = float(hp.get("lambda_ddis",       1.0))
        self.lambda_ladv     = float(hp.get("lambda_ladv",       0.0))  # 0 by default: remove competing gradient
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── A31 core: L1-d MEAN ALIGNMENT (constant gradient, no stalling) ────
        # Problem with A30: d² loss (Cohen's d²) had 49× larger per-step gradient
        # than A29's MSE, causing task collapse in ep0 (task AUC → 0.5).
        # ALSO: d² gradient ∝ Δ/σ² → stalls as Δ→0 (same problem as A29 MSE).
        # ALSO: lambda_ladv=1.0 contributed 0.559 competing gradient (3.8× task).
        #
        # Fix 1: L1-d loss = mean(|mu0-mu1| / std_combined.detach())
        #   Gradient = ±1/(n_per_class × std_combined × n_dims)  — CONSTANT.
        #   As Δ→0: std_combined→std_within (σ smaller) → gradient INCREASES.
        #   Self-accelerating near convergence. No stalling.
        #   Per-dim gradient at λ=0.05: 0.00271/step (31% of A29's 0.0088 → safe).
        #
        # Fix 2: lambda_ladv=0.0 — removes 0.559 competing gradient.
        #   task fraction improves from 22% (A30) to 72% (A31).
        if n0 >= 1 and n1 >= 1:
            mu0 = z_sync_pool[mask0].mean(0)   # [512]
            mu1 = z_sync_pool[mask1].mean(0)   # [512]
            with torch.no_grad():
                std_combined = z_sync_pool.std(0).clamp(min=1e-6)  # [512], detached
    ...
    def _run_domain_probe(self):
        ...
        def _probe(X, y, n_folds=5):
            # 5-fold stratified CV — avoids n<d overfitting with 400 samples, 512 dims
            pipe = make_pipeline(
                StandardScaler(),
...
def main():
    ...
    model = E2EModelA31(config=config)
    ...
    if finetune_ckpt and os.path.isfile(finetune_ckpt):
        print(f"[A31] Loading weights from {finetune_ckpt} (strict=False)", flush=True)
        ...
        print(f"[A31] Missing keys (new modules, OK): {missing[:10]}", flush=True)
        print(f"[A31] Unexpected keys (removed modules, OK): {unexpected[:5]}", flush=True)
        print(f"[A31] lambda_mean_align={model.lambda_mean_align}  lambda_ladv={model.lambda_ladv}  (L1-d loss)", flush=True)
    else:
        print("[A31] WARNING: no finetune_ckpt found — starting from scratch.", flush=True)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A31_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A31_e2e/ckpts"),
    ...
    print(">>> A31_e2e training complete.", flush=True)
