...
for both untrimmed and trimmed (leading-silence-removed) features.

Extends eval_caren_methods_12.py with:
  - --eval_dataset {favc, av1m}
  - --eval_features_root (switches trimmed vs untrimmed)
  - --ref_features_root  (AV1M val-real kNN reference; should match trimmed status)
  - For AV1M val eval: per-category AUC over (real, RV-FA, FV-RA, FV-FA) with
    kNN self-match exclusion when test paths overlap the reference.

All baselines (sup_alone, unsup_kNN, unsup_Mahal, unsup_sync), rank-fusion
variants (1a/1b/1c), logit-fusion grid (β), and residual-gated grid (α) are
preserved so the same JSON schema is emitted for FAVC and AV1M.
"""
...
def per_subset_auc_favc(scores, labels, cats, cat_to_idx):
    ...
    for fc in _FAVC_FAKE:
        ...
        if len(set(sub_lab)) < 2:
            res[_FAVC_ALIAS[fc]] = None
            continue
        res[_FAVC_ALIAS[fc]] = roc_auc_score(sub_lab, sub_score)
    return res
...
def per_subset_auc_av1m(scores, labels, cats, cat_to_idx):
    ...
    for alias in _AV1M_FAKE_ALIASES:
        ...
        if len(set(sub_lab)) < 2:
            res[alias] = None
            continue
        res[alias] = roc_auc_score(sub_lab, sub_score)
    return res
...
def main():
    ...
    print(f"{'Method':<32s} " + " ".join(f"{c:>8s}" for c in cols))
    print("-" * 80)
    for name, r in rows:
        def fmt(v): return f"{v:.4f}" if v is not None else "  n/a "
        vals = " ".join(fmt(r.get(c)).rjust(8) for c in cols)
