"""
E2E Training — A17 + Encoder-Level DANN (Gradient Reversal + Domain Classifier)
================================================================================

Root cause of A11/A12/A13/A14 failure:

  CORAL on n=4 samples from R^2048 is numerically rank-degenerate.
  The sample covariance is rank ≤3, so CORAL only constrains 3 eigendirections
  out of the full 2048-dimensional feature space.  The encoder learns to make
  these 3 directions equal while leaving the remaining 2045 directions
  unchanged — domain AUC stays at 1.0 despite low CORAL loss.

A17 fix: DANN (Domain-Adversarial Neural Network) at enc_feats level.

  Instead of 2nd-order statistics (CORAL), we use a LEARNED adversarial
  domain classifier directly on enc_feats:

    domain_clf_enc: [2048] → 256 → 64 → 1  (scalar domain logit)

  The classifier is trained to predict domain from enc_feats (i.e., to be
  100% accurate).  Via Gradient Reversal (GRL), the gradient is reversed
  before it reaches the encoder, forcing the encoder to FOOL the classifier.

  As training progresses:
    • Classifier attacks whichever directions best separate domains.
    • Encoder removes those directions to fool the classifier.
    • Classifier then finds new directions.  Arms race continues.
  End state: encoder can no longer produce domain-separable features.

  Advantages over CORAL:
    1. Works with n=4 per domain (no rank requirements)
    2. Attacks the FULL domain subspace iteratively (not just rank-3 cov)
    3. Nonlinear discrimination — catches higher-order domain signals
    4. Self-calibrating: as domain AUC drops, the gradient signal grows
       stronger (classifier becomes more uncertain → larger gradient)

  Monitor: train_domain_acc_enc should trend toward 0.5 (random guessing).
  When train_domain_acc_enc ≈ 0.5, encoder is domain-invariant.

  python train_e2e_a17.py --config_path configs/A17_e2e.yaml
"""
...
class E2EModelA17(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A17: Domain-Adversarial Neural Network (DANN) at encoder output level.
    Replaces rank-degenerate CORAL with adversarial GRL domain classifier.
    """
    ...
    def __init__(self, config: dict):
        ...
        # A17: domain classifier at enc_feats [2048] level with GRL
        ...
        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A17
    ...
    def configure_optimizers(self):
        ...
        head_params    = list(self.head.parameters()) + list(self.domain_clf_enc.parameters())
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Encoder-level DANN: GRL + domain classifier on enc_feats ─────────
        # The classifier (trained at lr_head) learns to detect domain from
        # enc_feats.  GRL reverses the gradient to AV-HuBERT, forcing the
        # encoder to produce domain-indistinguishable features.
        # Works with n=4 per domain (no rank requirement unlike CORAL).
        ...
        # Optional CORAL secondary term (lambda_enc_coral=0 in A17 config)
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [DANN]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA17(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A17_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A17_e2e/ckpts"),
    ...
    print(">>> A17_e2e training complete.", flush=True)
