"""
E2E Training — A13 + Dual-Level CORAL (Encoder + Z_sync), Fully End-to-End
=============================================================================

Difference from A11:
  A11: CORAL only on encoder output enc_feats [B,2048]
  A13: CORAL on BOTH encoder output AND Z_sync simultaneously
        loss = lambda_enc_coral * CORAL(enc_feats) + lambda_zsync_coral * CORAL(z_sync)

  Belt + suspenders: align at entrance to SAD head AND at SAD head output.
  Hypothesis: encoder-level CORAL fixes the upstream signal,
  Z_sync-level CORAL (as in A10) fixes any residual domain leaked by SAD head.

  python train_e2e_a13.py --config_path configs/A13_e2e.yaml
"""
...
class E2EModelA13(L.LightningModule):
    """
    ...
    def __init__(self, config: dict):
        ...
        self.lambda_zsync_coral = float(hp.get("lambda_zsync_coral", 25.0))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if n0 >= 2 and n1 >= 2:
            ...
            loss_zsync_coral = _coral(z_sync_pool[mask0],   z_sync_pool[mask1])
        else:
            loss_enc_coral   = torch.zeros(1, device=enc_feats.device).squeeze()
            loss_zsync_coral = torch.zeros(1, device=enc_feats.device).squeeze()
        ...
        loss = (
            ...
            + self.lambda_zsync_coral * loss_zsync_coral
        ...
        self.log("train_loss_zsync_coral",   loss_zsync_coral, **log_kw)
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [enc+zsync dual CORAL]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA13(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A13_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A13_e2e/ckpts"),
    ...
    print(">>> A13_e2e training complete.", flush=True)
