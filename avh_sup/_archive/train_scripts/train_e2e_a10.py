"""
E2E Training — A10 + CORAL Domain Alignment, Fully End-to-End
================================================================
...
Usage:
  python train_e2e_a10.py --config_path configs/A10_e2e.yaml
"""
...
class E2EModelA10(L.LightningModule):
    """
    End-to-end model: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    Identical loss to A9_e2e.  Domain data and augmentation fix is in load_data.
    """
    ...
    def __init__(self, config: dict):
        ...
        self.lambda_coral     = float(hp.get("lambda_coral",    25.0))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # CORAL: align mean + covariance of Z_sync across domains.
        ...
        if n0 >= 2 and n1 >= 2:
            loss_coral = _coral(z_sync_pool[mask0], z_sync_pool[mask1])
        else:
            loss_coral = torch.zeros(1, device=z_sync_pool.device).squeeze()
        ...
        self.log("train_loss_coral",     loss_coral,     **log_kw)
...
def main():
    ...
    model = E2EModelA10(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A10_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A10_e2e/ckpts"),
