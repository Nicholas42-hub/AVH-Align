"""
E2E Training — A12 + Stronger Encoder-Level CORAL (λ=100), Fully End-to-End
=============================================================================

Difference from A11:
  A11 used lambda_enc_coral=25.  A12 uses lambda_enc_coral=100.
  Test hypothesis: CORAL at encoder level needs stronger pressure to overcome
  the very strong domain signal baked in by AV-HuBERT pre-training (AUC=0.999).

  All other settings identical to A11:
  - CORAL directly on AV-HuBERT output enc_feats [B,2048]
  - BalancedDomainSampler (100% fire rate)
  - All FAVC real+fake, 3x oversample
  - Strong augmentation
  - Per-epoch domain probe

  python train_e2e_a12.py --config_path configs/A12_e2e.yaml
"""
...
class E2EModelA12(L.LightningModule):
    """
    ...
    def __init__(self, config: dict):
        ...
        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 100.0))
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [λ_enc=100]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA12(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A12_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A12_e2e/ckpts"),
    ...
    print(">>> A12_e2e training complete.", flush=True)
