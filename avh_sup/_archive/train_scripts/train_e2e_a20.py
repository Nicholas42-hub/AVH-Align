"""
E2E Training — A20 + Dual DANN Without Conflicting Domain Loss
================================================================================

A20 = A18 (dual DANN) with lambda_ddis=0.0.

Motivation:
  In A17/A18, there is a conflicting gradient at the encoder level:
    - loss_ddis (lambda=1.0): trains domain_head_s to predict domain from z_app.
      This pushes domain info INTO enc_feats (via backward through z_app → SAD head → encoder).
    - loss_enc_dann (GRL, lambda=1.0): pushes domain info OUT of enc_feats.

  These two losses fight each other with equal weight (both BCE, lambda=1.0).
  The net domain gradient may be near-zero, preventing the encoder from learning
  domain invariance.

  A20 removes this conflict by setting lambda_ddis=0.0, letting the dual DANN
  losses (enc + zsync) operate without interference.
  The orth constraint (z_sync ⊥ z_app, lambda_orth=0.1) is kept.

  python train_e2e_a20.py --config_path configs/A20_e2e.yaml
"""
...
class E2EModelA20(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A20: Dual DANN (enc_feats + z_sync_pool) without conflicting domain loss.
    lambda_ddis=0.0 removes the gradient that would PROMOTE domain info in encoder.
    """
    ...
    def __init__(self, config: dict):
        ...
        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A20
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── Z_sync-level DANN (A20 new): GRL on z_sync_pool ──────────────────
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [dual DANN, ddis=0]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA20(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A20_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A20_e2e/ckpts"),
    ...
    print(">>> A20_e2e training complete.", flush=True)
