"""
A8 — SAD: Synchrony-Appearance Decomposition training entry-point.

Usage:
  python train_test_sad_a8.py --config_path configs/A8.yaml --mode train

Architecture change from A5–A7:
  Replaces the FCD feature decomposition (CCD + URD + SDA) with a task-aware
  Synchrony-Appearance Decomposition (SAD):
    • Z_sync (causal):   cross-modal interaction cat(V,A,V⊙A,V−A) → projection
    • Z_app  (spurious): within-modal V-only + A-only appearance encoders

Training dataset:
  Domain 0 = AV1M train/val  (task-labelled, full supervision)
  Domain 1 = FAVC val-real   (no task label, domain signal only)
"""
...
# ─────────────────────────────────────────────────────────────────────────────
#  Domain-labelled wrapper datasets  (identical to A6/A7)
# ─────────────────────────────────────────────────────────────────────────────
...
def train(config: dict):
    ...
    model  = AVH_SAD_A8(config=config)
