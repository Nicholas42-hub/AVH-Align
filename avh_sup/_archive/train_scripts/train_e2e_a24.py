"""
E2E Training — A24: CDAN (Conditional Domain-Adversarial Network) on z_sync
================================================================================

**Motivation — why vanilla DANN (A17-A20) may fail**:
  In DANN, the domain classifier receives z_sync_pool [B,512] directly.
  The encoder can fool this classifier by rotating the representation into a
  domain-neutral subspace WHILE keeping a domain-discriminative component in
  the orthogonal subspace that the fixed-capacity classifier cannot capture.
  The adversarial game converges to a Nash equilibrium where the encoder
  still retains domain signal in directions the classifier ignores.

**CDAN fix (Long et al., NIPS 2018)**:
  Condition the domain classifier on the task prediction.
  Input to domain clf = z_sync_pool ⊙ sigmoid(task_score)  [element-wise]
  This "gates" each feature dimension by the task confidence.  Dimensions
  that are highly task-informative receive stronger domain alignment pressure.
  The classifier cannot be fooled by routing domain info through task-irrelevant
  dimensions — those are down-weighted and thus less useful to fool the clf.

  Formally: f_cdan(h, p) = h · p  where h = z_sync [B,512], p = sigmoid(score) [B,1]
  Domain clf input = h·p ∈ R^512  (broadcast multiply).  Gradient flows
  through h (z_sync → encoder) but NOT through p (detached) to avoid circular
  dependency where the task prediction is modified to defeat domain alignment.

**Architecture**:
  - CDAN domain clf on z_sync_pool (lambda_cdan=20.0, updated from 10.0)
  - Enc-level DANN on enc_feats (lambda_enc_dann=15.0) for upstream pressure
    (updated from 10.0 based on A18 regression showing lambda=1 too weak)
  - No z_sync vanilla DANN (mechanism isolation vs A24 vs A20)
  - lambda_ddis=0.0, lambda_orth=0.1
...
class E2EModelA24(L.LightningModule):
    """
    End-to-end: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    A24: CDAN (Conditional Domain-Adversarial Network) at z_sync_pool level.

    Domain classifier input = z_sync_pool * sigmoid(task_score).unsqueeze(-1)
    where task_score is computed for ALL samples (incl. FAVC) with detach() on
    the task probability to avoid circular gradient dependency.

    Additional enc-level DANN (lambda=10) provides upstream pressure so that
    enc_feats themselves also become domain-invariant over time.

    lambda_ddis=0.0: no conflicting domain gradient from z_app path.
    """
    ...
    def __init__(self, config: dict):
        ...
        # A24: CDAN domain classifier on z_sync_pool [B,512] conditioned on task
        # Input dimension same as z_sync: 512 (task_prob is scalar, broadcast-mul)
        zsync_dim = int(hp.get("sync_dim", 512))  # 512
        self.domain_clf_zsync = _make_adv_head(zsync_dim)
        self.lambda_cdan      = float(hp.get("lambda_cdan",      10.0))
        self.grl_alpha_zsync  = float(hp.get("grl_alpha_zsync",   1.0))

        # No vanilla z_sync DANN in A24 (replaced by CDAN)
        self.lambda_zsync_dann = 0.0  # disabled (mechanism isolation)

        self.lambda_enc_coral = float(hp.get("lambda_enc_coral", 0.0))  # disabled in A24
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Prevents the encoder from building domain-discriminative enc_feats.
        ...
        # ── CDAN at z_sync_pool level (A24 core contribution) ────────────────
        # Step 1: get task score for ALL samples (including FAVC with label=-1)
        #   task_prob [B,1] = sigmoid(causal_head(z_sync)) via logsumexp.
        #   Detach task_prob to prevent domain classifier from manipulating
        #   task predictions to defeat domain alignment (circular dependency).
        # Step 2: condition z_sync on task prediction
        #   cdan_feat [B,512] = z_sync_pool * task_prob  (broadcast multiply)
        # Step 3: GRL on cdan_feat -> domain classifier -> BCE
        with torch.no_grad():
        ...
        with torch.no_grad():
            domain_acc_zsync = ((domain_logit_cdan.detach() > 0).float() == domain_labels).float().mean()
    ...
    def on_validation_epoch_end(self):
        ...
        if self.domain_probe_loader is not None:
            ...
            print(f"\n[Epoch {epoch:02d}] Domain probe: "
                  ...
                  f"(target <0.65) [CDAN + enc DANN lambda=10, ddis=0]  task AUC={auc:.4f}",
                  flush=True)
...
def main():
    ...
    model = E2EModelA24(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A24_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A24_e2e/ckpts"),
    ...
    print(">>> A24_e2e training complete.", flush=True)
