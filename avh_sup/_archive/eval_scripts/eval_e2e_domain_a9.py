"""
E2E Domain & Label Probe Evaluation — A9_e2e (SAD + MMD)
=========================================================
Evaluates an E2EModelA9 checkpoint on:
  1. Domain predictability (AV1M=0 vs FakeAVCeleb=1) from Z_sync and Z_app
     → target: Z_sync AUC ≈ 0.5  (MMD expelled domain through encoder)
  2. Label AUC (real=0 vs fake=1) from Z_sync and Z_app on FAVC (OOD)
     → tests whether Z_sync generalises out-of-domain

Unlike frozen-encoder eval scripts (which use pre-extracted .npz files),
this feeds raw preprocessed video/audio through the full e2e pipeline.
...
Comparison baselines (frozen encoders):
  ...
  A9_e2e Z_sync target:   domain AUC ≪ 0.9   (MMD pushes through encoder)

Usage:
  python eval_e2e_domain_a9.py \\
      --ckpt      avh_sup/outputs_A9_e2e/ckpts/model-epoch=XX-val_auc_causal=X.ckpt \\
      --config    avh_sup/configs/A9_e2e.yaml \\
      --av1m_raw  /data/scratch/.../avd1m_preprocessed \\
      --favc_raw  /data/scratch/.../favc_preprocessed \\
      --av1m_csv  avh_sup/csv_metadata/av1m \\
      --output_dir avh_sup/outputs_A9_e2e/results \\
...
from train_e2e_a9 import E2EModelA9
...
@torch.no_grad()
def extract_zc_zs(model: E2EModelA9, dataloader: DataLoader,
...
def main():
    ...
    parser.add_argument("--output_dir",  default="outputs_A9_e2e/results")
    ...
    model = E2EModelA9.load_from_checkpoint(args.ckpt, config=config)
    model.eval()
    model.to(device)
    ...
    print("  Raw AV-HuBERT Z_sync domain AUC ≈ 0.999")
    print("  Frozen A9     Z_sync domain AUC ≈ 0.988  Δraw = −0.011")
    print("  A9_e2e        Z_sync domain AUC = ???    target: ≪ 0.9")
