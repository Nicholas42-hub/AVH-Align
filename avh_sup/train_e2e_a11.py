"""
E2E Training — A11 + Encoder-Level CORAL Domain Alignment, Fully End-to-End
=============================================================================

Key difference from A10:

  A10 applied CORAL on Z_sync (512-dim output of SAD head).
  Gradient path: CORAL loss → SAD head → AV-HuBERT.

  A11 applies CORAL *directly on the AV-HuBERT encoder output*:
    enc_feats = cat(video_feats.mean(T), audio_feats.mean(T))  [B, 2048]
    loss_enc_coral = CORAL(enc_feats[domain==0], enc_feats[domain==1])

  Gradient path: CORAL loss → AV-HuBERT directly, with no SAD head in between.
  This is the most direct possible signal telling AV-HuBERT:
  "your output features must look the same for AV1M and FAVC clips."

  Hypothesis: upstream alignment (at encoder output) may be more effective
  than downstream alignment (at task-head output) because:
    1. The domain signal is baked into AV-HuBERT features → fix it there
    2. The gradient path is shorter → cleaner signal to the encoder
    3. Z_sync already has the grad from task loss competing; encoder has more freedom

Root cause analysis of A9/A9_e2e failure:

  ROOT CAUSE 1 — MMD almost never fired:
    A9_e2e: FAVC real_only=True → 500 real clips × 10× = 5000 samples.
    AV1M: 59,884 clips.  FAVC fraction per batch = 7.7%.
    At batch_size=8:  P(≥2 FAVC clips) ≈ 11%  → MMD skipped in 89% of steps.
    The domain alignment loss was effectively absent.

  ROOT CAUSE 2 — Manipulation-distribution confound:
    Using real-only FAVC as domain=1 vs AV1M (mostly fake) as domain=0
    means MMD was aligning "all-real features" vs "mostly-fake features".
    Deepfakes SHOULD have different A-V synchrony → MMD fought the task loss.

  ROOT CAUSE 3 — Input-level domain gap:
    AV1M (YouTube) vs FAVC differ at pixel/audio level (codec, demographics).

Fixes in A10:
  FIX 1 — BalancedDomainSampler: guarantees EXACTLY 50% AV1M / 50% FAVC
    in every batch, so the domain alignment loss fires on every step.

  FIX 2 — All FAVC clips (real+fake, ~21k, real_only=False): both domains
    have matched real+fake manipulation distributions; MMD targets recording
    conditions only.

  FIX 3 — CORAL domain alignment loss (replaces multi-kernel MMD):
    Simply aligns mean and covariance of Z_sync across domains.  No
    adversarial game, no saddle point, always non-zero gradient.
    In 5 lines it directly attacks what a linear probe exploits.

  FIX 4 — Strong input augmentation: Gaussian noise + brightness + flip
    on BOTH domains to blur input-level recording-condition gaps.

Usage:
  python train_e2e_a11.py --config_path configs/A11_e2e.yaml
"""
...
class E2EModelA11(L.LightningModule):
    """
    End-to-end model: AVHubertWrapper (fully unfrozen) + AVH_SAD_A9 head.
    Key change vs A10: CORAL is applied directly on AV-HuBERT encoder output
    (video_feats + audio_feats, mean-pooled over time) rather than on Z_sync.
    This is a direct, short-gradient-path signal to the encoder itself.
    """
...
def main():
    ...
    model = E2EModelA11(config=config)
    ...
    logger = CSVLogger(
        save_dir = log_cfg.get("log_path", "outputs_A11_e2e/logs"),
        name     = "",
    )
    ckpt_cb = ModelCheckpoint(
        dirpath    = ckpt_cfg.get("ckpt_dir",  "outputs_A11_e2e/ckpts"),
    ...
    print(">>> A11_e2e training complete.", flush=True)
