...
-----------------------
Both categories share the same video manipulation (face swap / deepfake video).
The ONLY difference is whether the audio is real (0) or synthesised/swapped (1).

If disentanglement is working:
  Z_s AUC >> raw   — spurious branch is sensitive to audio manipulation style
  Z_c AUC ≈ raw    — causal (A-V sync) branch is blind to audio swap type

Unlike the domain probe (AV1M vs FAVC), this task is WITHIN a single dataset,
so the upstream AV-HuBERT raw AUC is not expected to be near 100%.
Any elevation of Z_s AUC above raw is directly attributable to the model.

Compares across ablations (A0, A1, A2, A3) to show the effect of each component.
...
class ManipDataset(Dataset):
    """
    Clips from two FakeVideo categories, mean-pooled over time.
      label 0 = FakeVideo-RealAudio
      label 1 = FakeVideo-FakeAudio
    """
...
def extract_raw(dataloader: DataLoader):
    """Raw AV-HuBERT features — upstream baseline, no model."""
    all_raw, all_labels = [], []
...
def extract_repr(model: AVH_Causal_Ablation, dataloader: DataLoader,
                 device: torch.device):
    """Extract Z_c and Z_s from trained model."""
    model.eval()
    model.to(device)
...
def linear_probe_auc(features, labels, n_splits=5, seed=42):
    ...
    for train_idx, test_idx in skf.split(features, labels):
        X_train = StandardScaler().fit(features[train_idx]).transform(features[train_idx])
...
def main():
    parser = argparse.ArgumentParser(
        description="Manipulation-type probe: FakeVideo-RealAudio vs FakeVideo-FakeAudio")
    ...
    print("    Δraw(Z_s) >> 0  (Z_s captures audio manipulation style)")
    print("    Δraw(Z_c) ≈  0  (Z_c is blind to audio swap type)")
    print()
