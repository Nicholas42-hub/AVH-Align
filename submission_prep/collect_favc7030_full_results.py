#!/usr/bin/env python3
"""Collect A2/A5/A6 seed-sweep results on full FakeAVCeleb 70/30 test."""
...
RESULT_SUBDIR = "results_favc_7030_full"
...
def parse_eval_results(path: Path) -> dict[str, dict[str, float]] | None:
    ...
    for line in path.read_text().splitlines():
        line = line.rstrip()
        if "-- FULL HEAD --" in line or "── FULL HEAD ──" in line:
            current_head = "full"
            continue
        if "-- CAUSAL HEAD --" in line or "── CAUSAL HEAD ──" in line:
            current_head = "causal"
            continue
        if "-- SPURIOUS HEAD --" in line or "── SPURIOUS HEAD ──" in line:
...
def main() -> None:
    ...
    raw_path = out_dir / "favc7030_full_seed_results_raw.csv"
    with raw_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=raw_fieldnames)
        writer.writeheader()
        writer.writerows(raw_rows)

    summary_path = out_dir / "favc7030_full_seed_results_summary.csv"
